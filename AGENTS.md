...
### `@architect`
Read-only analyst. Explores the codebase, understands existing patterns, and produces structured implementation plans by creating GitHub issues for tracking. Plans must include estimated AI time (time the AI will spend implementing the complete flow) and estimated token usage. Cannot edit files or run commands. Reviews the tester's test spec (step 4) and the developer's implementation (step 6) before passing work forward.
...
### `@developer`
Implements code changes against the tester's test spec. Edits source files and runs build/compile commands autonomously. When work is complete, presents a summary of all changes and asks for human review. On failure, retries up to 3 times before escalating to the user.
